...
'''
Filter that produces XSD tree - adjusted from html.py
@author Sergey Chikuyonok (serge.che@gmail.com) - altered by Dave Brotherstone
...
def make_attributes_string(tag, profile):
	...
	id_attr_valid = False
	name_attr_valid = False
	
	for a in tag.attributes:
		if a['name'] == 'id' and a['value']:
			id_attr_valid = True
		elif a['name'] == 'name' and a['value']:
			name_attr_valid = True
	...
	for a in tag.attributes:
		if a['name'] == 'id':
			if id_attr_valid and not name_attr_valid:
				a['name'] = 'name'
		elif a['name'] == 'class':
			a['name'] = 'type'
		elif a['name'] == 'name':
		...
		attrs += ' ' + a['name'] + '=' + attr_quote + (a['value'] or cursor) + attr_quote
